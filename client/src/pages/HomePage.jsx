...
import { Link } from "react-router-dom";
import { getAllMedia } from "../services/api"; // Your existing API function
import { gsap } from "gsap";
import { ScrollTrigger } from "gsap/ScrollTrigger";
...
const HomePage = () => {
  ...
  useEffect(() => {
    getAllMedia()
      .then((res) => {
        const onlyImages = res.data.filter(item => item.type.includes("image"));
        setGalleryImages(onlyImages.map(item => item.url));
      })
  ...
  useEffect(() => {
    ...
    gsap.fromTo(paraRef.current, { opacity: 0, y: 40 }, {
      opacity: 1, y: 0, duration: 1, delay: 0.3,
      scrollTrigger: { trigger: paraRef.current, start: "top 80%", once: true }
    });

    gsap.fromTo(btnRef.current, { opacity: 0, scale: 0.9 }, {
      opacity: 1, scale: 1, duration: 0.8, delay: 0.5, ease: "back.out(1.7)",
      scrollTrigger: { trigger: btnRef.current, start: "top 80%", once: true }
    });

    const images = imageSectionRef.current?.querySelectorAll("img") || [];
    gsap.fromTo(images, { opacity: 0, y: 50 }, {
      opacity: 1, y: 0, stagger: 0.2,
      scrollTrigger: { trigger: imageSectionRef.current, start: "top 80%", once: true }
    });
  }, [galleryImages]);
  ...
  return (
    <div className="min-h-screen bg-black text-white px-6 text-center relative">
      ...
      {/* Recents Heading */}
      <section className="mt-20 text-left pb-4 max-w-6xl mx-auto px-4">
        ...
        >
          {galleryImages.slice(0, 6).map((src, i) => (
            <img
              ...
              src={src}
              alt={`Gallery ${i}`}
              className="rounded-xl shadow-md hover:scale-105 transition duration-300 object-cover"
            />
          ))}
        </div>
      </section>
